// ── ARCHIVE MODE ─────────────────────────────────────────────────────────────
// Data is loaded from static JSON exports. Supabase auth, live queries, and
// form submissions are all commented out below for reference.
...
// Skip login gate — show archived data directly
if (loginGate)     loginGate.style.display    = "none";
if (portalContent) portalContent.style.display = "table";

Promise.all([
  fetch("data/rsvps.json").then(r => r.json()),
  fetch("data/comments.json").then(r => r.json())
]).then(([rsvps, comments]) => {
  renderAttendees(rsvps.filter(r => r.attending === "yes" || r.attending === "maybe"));
  renderComments(comments);
}).catch(err => {
  if (attendeeList) attendeeList.innerHTML = `<li>Could not load archived data: ${err.message}</li>`;
  if (commentList)  commentList.innerHTML  = `<li>Could not load archived data: ${err.message}</li>`;
});

function renderAttendees(data) {
  if (!attendeeList) return;
  if (!data.length) { attendeeList.innerHTML = "<li>No responses.</li>"; return; }
...
function renderComments(data) {
  ...
  if (!data.length) { commentList.innerHTML = "<li>No comments.</li>"; return; }

  const seedSrc = "assets/single_dandelion_seed.svg";
  commentList.innerHTML = "";
  data
    .sort((a, b) => (b.wishes_caught || 0) - (a.wishes_caught || 0))
    .forEach((row, index) => {
      const item = document.createElement("li");

      const info = document.createElement("span");
      info.className = "attendee-info";
      info.textContent = `#${index + 1} ${row.name}: ${row.comment}`;
...
if (savedPass) initPortal(savedPass);
...
async function initPortal(pass) {
  ...
  if (!normalizedPass) { if (loginError) loginError.textContent = "Enter the authorization code."; return; }
  if (!cfg.supabaseUrl || !cfg.supabaseAnonKey) { if (loginError) loginError.textContent = "Supabase config missing."; return; }
  if (loginError) loginError.textContent = "Verifying...";

  supabase = createClient(cfg.supabaseUrl, cfg.supabaseAnonKey, {
    auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false },
    global: { headers: { "x-portal-password": normalizedPass } }
  });
...
function wireHandlers(supabase) {
  ...
  handlersWired = true;
  if (!rsvpForm || !commentForm || !rsvpStatus || !commentStatus) return;
  ...
  rsvpForm.addEventListener("submit", async (event) => {
    ...
    const payload = {
      ...
      guests: formData.get("bringing_plus") === "on" ? 1 : 0,
    ...
    if (error) { rsvpStatus.textContent = `Could not submit RSVP: ${error.message}`; return; }
    rsvpForm.reset();
  ...
  commentForm.addEventListener("submit", async (event) => {
    ...
    if (error) { commentStatus.textContent = `Could not post comment: ${error.message}`; return; }
    commentForm.reset();
...
async function refreshAttendees(supabase) {
  ...
  const { data, error } = await supabase
    ...
    .limit(100);
  if (error) { attendeeList.innerHTML = `<li>Could not load attendees: ${error.message}</li>`; return; }
  if (!data.length) { attendeeList.innerHTML = "<li>No responses yet.</li>"; return; }
  attendeeList.innerHTML = "";
  data.forEach((row) => {
    const item = document.createElement("li");
    const messageSuffix = row.message ? ` — "${row.message}"` : "";
...
async function refreshComments(supabase) {
  ...
  const { data, error } = await supabase
    ...
    .order("created_at",   { ascending: false })
    .limit(100);
  if (error) { commentList.innerHTML = `<li>Could not load comments: ${error.message}</li>`; return; }
  if (!data.length) { commentList.innerHTML = "<li>No comments yet.</li>"; return; }
  // ... seed rendering logic moved to renderComments() above
}
...
function wireHandlers(supabase) {
  ...
  rsvpForm.addEventListener("submit", async (event) => {
    ...
    const formData = new FormData(rsvpForm);
    const isBringingPlusOne = formData.get("bringing_plus") === "on";
    const payload = {
      name: String(formData.get("name") || "").trim(),
      email: normalizeOptional(formData.get("email")),
      attending: String(formData.get("attending") || "maybe"),
      guests: isBringingPlusOne ? 1 : 0,
      message: normalizeOptional(formData.get("message")),
      private_message: normalizeOptional(formData.get("private_message"))
    };
